    ...
    public String run(int mode) throws Exception {
        ...
        if (mode == 0) {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                w.run();
                if (!w.isValid()) finalValid = false;
                finalErrors.addAll(w.getErrors());
            }

            for (int j = 0; j < 9; j++) {
                ColChecker w = new ColChecker(board, j);
                w.run();
                if (!w.isValid()) finalValid = false;
                finalErrors.addAll(w.getErrors());
            }
            ...
            for (int i = 0; i < 9; i++) checkers[idx++] = new RowChecker(board, i);
            for (int j = 0; j < 9; j++) checkers[idx++] = new ColChecker(board, j);
            for (int b = 0; b < 9; b++) checkers[idx++] = new BoxChecker(board, b);

            for (int i = 0; i < 27; i++) threads[i] = new Thread(checkers[i]);

            for (Thread t : threads) t.start();
            for (Thread t : threads) t.join();

            for (Checker c : checkers) {
                if (!c.isValid()) finalValid = false;
        ...
        if (finalValid) return "VALID\n";

        StringBuilder sb = new StringBuilder("INVALID\n");
        for (String e : finalErrors) sb.append(e).append("\n");
        return sb.toString();
    }
}
